...
# Libraries
source("./gra_tra_mul_mar_lik_GPP.R")
...
Ckk  <- exp(- ph * Dkk)
Cks  <- exp(- ph * Dks)
A    <- s2 * (ga * t(Cks) %*% solve(Ckk, Cks) + diag(nrow = N) / K)
eig  <- eigen(A)

# Test ga
Ckkh <- exp(- ph * Dkk)
Cksh <- exp(- ph * Dks)
Ah   <- s2 * (gah * t(Cksh) %*% solve(Ckkh, Cksh) + diag(nrow = N) / K)
eigh <- eigen(Ah)
...
Ckkh <- exp(- phh * Dkk)
Cksh <- exp(- phh * Dks)
Ah   <- s2 * (ga * t(Cksh) %*% solve(Ckkh, Cksh) + diag(nrow = N) / K)
eigh <- eigen(Ah)
...
tim <- Sys.time()
for(i in 1:100){
  out5 <- gra_tra_mul_mar_lik_GPP(s2  = s2,
                                  ga  = ga,
                                  ph  = ph,
                                  yb  = yb,
                                  Dkk = Dkk,
